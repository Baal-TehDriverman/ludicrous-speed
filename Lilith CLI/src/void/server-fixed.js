 ...
 * The mod runtime was never being integrated — this fixes the import path.
 */
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (mode === 'python') {
        ...
        child = spawn('python3', [tmpFile], { cwd: process.cwd(), env: { ...process.env, NODE_PATH: NODE_MODULES, VOID_PROFILE: profile, PYTHONUNBUFFERED: '1' }, timeout: Math.min(timeoutMs, MAX_TIMEOUT_MS), stdio: ['pipe', 'pipe', 'pipe'] });
      } else {
        child = spawn('node', ['-e', code], { cwd: process.cwd(), env: { ...process.env, NODE_PATH: NODE_MODULES, VOID_PROFILE: profile }, timeout: Math.min(timeoutMs, MAX_TIMEOUT_MS), stdio: ['pipe', 'pipe', 'pipe'] });
      }
      let stdout = '', stderr = '';
...
export function createVoidApp() {
  ...
  app.post('/api/void/exec', async (req, res) => {
    ...
    res.json(await executeCode(code, effectiveMode, profile, timeout_ms));
  });
  ...
  // ─── Mod Runtime API — FIXED INTEGRATION ───
  app.use('/api/mod', (req, res, next) => {
    if (modAppRef) { modAppRef(req, res, next); } else { res.status(503).json({ error: 'Mod runtime not initialized' }); }
  });

  // Lazy-load mod runtime to avoid circular dependency
  setTimeout(async () => {
    try {
      const { createModApp } = await import('./modding/void-mod-runtime.js');
      modAppRef = createModApp();
      console.log(chalk.green('🜏 Mod runtime integrated into Void server'));
    } catch (err) {
