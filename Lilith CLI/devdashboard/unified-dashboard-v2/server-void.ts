...
import { mkdtempSync, rmSync, writeFileSync, appendFileSync, readFileSync, existsSync } from "fs";
...
const VOID_HISTORY_PATH = resolve(process.env.VOID_HISTORY_PATH || join(VOID_ROOT, "..", "void-history.jsonl"));
const VOID_HISTORY_MAX = parseInt(process.env.VOID_HISTORY_MAX || "50", 10);
...
async function acquireSlot(): Promise<() => void> {
  ...
  return new Promise<() => void>((resolve) => execQueue.push(resolve)).then(() => {
    voidExecActive++;
    return () => { voidExecActive--; pumpQueue(); };
  });
}
...
function appendHistory(entry: Record<string, unknown>) {
  try {
    appendFileSync(VOID_HISTORY_PATH, JSON.stringify(entry) + "\n");
    // Trim if too long
    if (existsSync(VOID_HISTORY_PATH)) {
      const lines = readFileSync(VOID_HISTORY_PATH, "utf-8").split("\n").filter(Boolean);
      if (lines.length > VOID_HISTORY_MAX) {
        writeFileSync(VOID_HISTORY_PATH, lines.slice(-VOID_HISTORY_MAX).join("\n") + "\n");
      }
    }
  } catch {}
}
...
export function registerVoidRoutes(app: import("express").Express) {
  ...
  app.get("/api/void/history", (_req: import("express").Request, res: import("express").Response) => {
    try {
      if (!existsSync(VOID_HISTORY_PATH)) {
        return res.json({ executions: [], total: 0 });
      }
      const lines = readFileSync(VOID_HISTORY_PATH, "utf-8").split("\n").filter(Boolean);
      const executions = lines.slice(-VOID_HISTORY_MAX).map((line) => JSON.parse(line));
  ...
  app.post("/api/void/exec", async (req: import("express").Request, res: import("express").Response) => {
    ...
    child.on("close", (codeNum, signal) => {
      ...
      if (!settled) {
        ...
        const result = {
          success: codeNum === 0,
          output: stdout.slice(-20000) || undefined,
          error: stderr.slice(-20000) || undefined,
          execution_time_ms: timeoutMs,
          mode,
          profile,
        };
        appendHistory({ ...result, timestamp: new Date().toISOString() });
        res.json(result);
      }
    });
  });
