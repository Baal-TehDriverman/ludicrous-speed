...
async function hyatlasStatusCmd() {
  try {
    const r2 = await hyatlasStatus();
    console.log(chalk.bold("\n\u{1F70F} HyAtlas Memory Status\n"), chalk.green(`  Status: ${r2.status}`), chalk.cyan(`  Embed: ${r2.data?.embed}`), chalk.cyan(`  LLM: ${r2.data?.llm}`), chalk.gray(`  Layers: ${JSON.stringify(r2.data?.layers, null, 2)}
`));
  } catch (e2) {
    console.log(chalk.red(`  Error: ${e2.message}`));
  }
}
async function hyatlasHealthCmd() {
  try {
    const r2 = await hyatlasHealth();
    console.log(chalk.bold("\n\u{1F70F} HyAtlas Health\n"), chalk.green(`  Status: ${r2.status}
`));
  } catch (e2) {
    console.log(chalk.red(`  Error: ${e2.message}`));
  }
...
async function hyatlasWriteCmd(text, options) {
  ...
  try {
    const r2 = await hyatlasAdd(text, options.user, options.agent, options.session);
    console.log(chalk.green(`  \u2713 Memory added: ${JSON.stringify(r2.data).slice(0, 200)}
`));
  } catch (e2) {
    console.log(chalk.red(`  \u2717 Failed: ${e2.message}
`));
  }
}
async function hyatlasSearchCmd(query, options) {
  if (!query) {
    console.log(chalk.yellow('  Usage: lilith void hyatlas search "query"'));
    return;
  }
  console.log(chalk.gray(`
...
`));
  try {
    const r2 = await hyatlasSearch(query, parseInt(options.limit), options.layer);
    console.log(chalk.bold("\n\u{1F70F} HyAtlas Search\n"));
    for (const [layer, memories] of Object.entries(r2.data?.memories || r2.memories || {})) {
      if (memories.length > 0) {
        console.log(chalk.cyan(`  ${layer}:`));
        for (const m2 of memories) console.log(chalk.gray(`    [${m2.memory_id}] ${m2.content.substring(0, 80)}...`));
      }
    }
    console.log();
  } catch (e2) {
...
async function hyatlasListCmd(options) {
  try {
    const r2 = await hyatlasList(parseInt(options.limit), options.layer);
    console.log(chalk.bold("\n\u{1F70F} HyAtlas Memories\n"));
    for (const m2 of r2.data?.memories || r2.memories || []) console.log(chalk.gray(`  [${m2.memory_id}] ${m2.content.substring(0, 80)}...`));
    console.log();
  } catch (e2) {
    console.log(chalk.red(`  Error: ${e2.message}`));
  }
...
async function hyatlasGraphCmd() {
  try {
    const r2 = await hyatlasGraph();
    console.log(chalk.bold("\n\u{1F70F} HyAtlas Graph\n"), chalk.cyan(`  Nodes: ${r2.data?.nodes?.length || 0}`), chalk.cyan(`  Edges: ${r2.data?.edges?.length || 0}
`));
  } catch (e2) {
    console.log(chalk.red(`  Error: ${e2.message}`));
  }
...
async function hyatlasDigestCmd() {
  try {
    const r2 = await hyatlasDigest();
    console.log(chalk.bold("\n\u{1F70F} HyAtlas Digest\n"), chalk.green(`  Status: ${r2.data?.status || "ok"}
`));
  } catch (e2) {
    console.log(chalk.red(`  Error: ${e2.message}`));
  }
...
async function hyatlasShellCmd(command) {
  if (!command) {
    console.log(chalk.yellow('  Usage: lilith void hyatlas shell "command"'));
    return;
  }
  try {
    const r2 = await hyatlasShell(command);
    console.log(chalk.bold("\n\u{1F70F} HyAtlas Shell\n"), chalk.gray(`  ${r2.data?.output || "ok"}
`));
  } catch (e2) {
    console.log(chalk.red(`  Error: ${e2.message}`));
  }
...
async function hyatlasStatsCmd() {
  try {
    const r2 = await hyatlasStats();
    console.log(chalk.bold("\n\u{1F70F} HyAtlas Stats\n"), chalk.cyan(`  Total Memories: ${r2.data?.totalMemories || 0}`), chalk.cyan(`  Uptime: ${r2.data?.uptime || 0}s
`));
  } catch (e2) {
    console.log(chalk.red(`  Error: ${e2.message}`));
  }
...
async function hyatlasDoctorCmd() {
  ...
  if (reachable.reachable) {
    const s2 = await hyatlasStatus();
    console.log(chalk.green(`  Status: ${s2.status}`), chalk.cyan(`  Embed: ${s2.data?.embed}`), chalk.cyan(`  LLM: ${s2.data?.llm}`), chalk.cyan(`  Layers: ${JSON.stringify(s2.data?.layers)}
`));
  } else {
    console.log(chalk.yellow("  Start with: hyatlas-go\n"));
  }
}
...
async function voidExecCmd(code, options) {
  if (!code) {
    console.log(chalk2.yellow(`  Usage: lilith void exec "console.log('hello')"`));
    return;
  }
  try {
    ...
    if (data.success) {
      console.log(chalk2.green(`  \u2713 Output: ${data.output}`));
    } else {
      console.log(chalk2.red(`  \u2717 Error: ${data.output}`));
    }
  } catch (e2) {
    console.log(chalk2.red(`  \u2717 Void server unreachable: ${e2.message}`));
  }
}
async function voidPythonCmd(script, options) {
  if (!script) {
    console.log(chalk2.yellow('  Usage: lilith void python "import numpy as np; print(np.array([1,2,3]))"'));
    return;
  }
  try {
    const res = await fetch2(`${VOID_API}/exec`, {
    ...
    const data = await res.json();
    if (data.success) {
      console.log(chalk2.green(`  \u2713 Python Output:
${data.output}`));
    } else {
      console.log(chalk2.red(`  \u2717 Python Error:
${data.output}`));
    }
  } catch (e2) {
    console.log(chalk2.red(`  \u2717 Void server unreachable: ${e2.message}`));
...
`;
  console.log(chalk2.cyan("  \u{1F70F} Rendering Black Hole Sun \xB7 Flower of Life \xB7 Pentagram..."));
  console.log(chalk2.gray("  This will take ~30-60 seconds"));
  try {
    const res = await fetch2(`${VOID_API}/exec`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const data = await res.json();
    if (data.success) {
      console.log(chalk2.green(`  \u2713 ${data.output}`));
    } else {
      console.log(chalk2.red(`  \u2717 Error: ${data.output}`));
    }
  } catch (e2) {
  ...
  try {
    ...
    if (data.history && data.history.length > 0) {
      for (const entry of data.history.slice(-10)) {
        console.log(chalk2.gray(`  [${entry.timestamp}] ${entry.mode} (${entry.profile}) - ${entry.success ? "OK" : "FAIL"}`));
        console.log(chalk2.gray(`    ${entry.output?.substring(0, 100)}...`));
      }
    } else {
      console.log(chalk2.gray("  No history"));
    }
    console.log();
  } catch (e2) {
    console.log(chalk2.red(`  \u2717 Void server unreachable: ${e2.message}`));
...
async function voidServerCmd(action = "status") {
  try {
    ...
    console.log(chalk2.cyan(`  Status: ${data.status}`));
    console.log(chalk2.cyan(`  Runtime: ${data.runtime_root}`));
    console.log(chalk2.cyan(`  Node: ${data.node_version} | V8: ${data.v8_version}`));
    console.log(chalk2.cyan(`  Active: ${data.active_executions} | Queued: ${data.queued} | Max: ${data.max_concurrent}`));
    console.log(chalk2.cyan(`  Profiles: ${data.profiles.join(", ")}`));
    console.log(chalk2.cyan(`  History: ${data.history_count} entries`));
    console.log(chalk2.cyan(`  Uptime: ${data.uptime_seconds}s`));
    console.log();
  } catch (e2) {
    if (action === "start") {
      console.log(chalk2.yellow("  Starting Void server..."));
      console.log(chalk2.gray("  Run: node server-fixed.js"));
    } else {
      console.log(chalk2.red(`  \u2717 Void server unreachable: ${e2.message}`));
    }
  }
...
async function voidConsoleCmd() {
  ...
  console.log(chalk2.dim("  Type JS code. Commands: .exit .status .mode .profile\\n"));
  const readline = await import("readline");
  const rl = readline.createInterface({
    input: process.stdin,
    output: process.stdout,
    prompt: chalk2.magenta("void> ")
  });
  let mode = "eval";
  let profile = "no-net";
  async function execute(code) {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        if (data.output) console.log(chalk2.green(data.output));
      } else {
        if (data.error) console.log(chalk2.red(data.error));
      }
    } catch (e2) {
      console.log(chalk2.red(`Error: ${e2.message}`));
    }
  }
  rl.prompt();
  rl.on("line", async (line) => {
    ...
    if (input.startsWith(".mode ")) {
      mode = input.split(" ")[1];
      console.log(chalk2.yellow(`Mode: ${mode}`));
      rl.prompt();
      return;
    }
    if (input.startsWith(".profile ")) {
      profile = input.split(" ")[1];
      console.log(chalk2.yellow(`Profile: ${profile}`));
      rl.prompt();
      return;
    }
    await execute(input);
...
async function voidGuiCmd() {
  console.log(chalk2.cyan("  Opening Void GUI at http://localhost:3000/api/void/gui"));
  console.log(chalk2.gray("  (This opens the web dashboard \u2014 use CLI for actual work)"));
}
